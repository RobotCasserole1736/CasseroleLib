...
/**
 * Generic Finite Impluse Response (FIR) filter. Constructor takes an argument to indicate what sort of filter to implement. 
 * Underlying logic remains the same for all filters, but coefficent value and number change to implement different
 * cutoff frequencies and styles.
 * <br><br>
 * A FIR filter is effectively a weighted average of past signal values, but with specifically chosen weights to
 * allow or disallow specific frequencies through. Lowpass filters allow low frequencies but block high frequencies,
 * while Highpass filters allow higher frequencies but block lower ones. The number indicates the aproximate frequency
 * at which the filter transitions between blocking and not blocking.
 * <br><br>
 * FIR filters must process samples at a very regular rate to maintain accuracy. For this reason, this implementation
 * outsources the computation of filter value to a separate task, called at a 20ms rate (50Hz). This limits the max.
 * frequency that can be processed by the filter to 25Hz (per the Nyquist Criteria).
 *
 *
 */
public class FIRFilter {
	
	/** Type of filtering this filter is doing. If for some reason you forgot what type it was, check it here */
	...
	/**
	 * Set a new input into the filter. Whatever is passed here will be picked up next time
	 * the filter task calculates the filter's value. 
	...
	/**
	 * FIR filter accuracy is highly dependent upon inputs being processed at a regular rate. In general
	 * our code kinda does that, but to absolutely ensure it, we'll fire off the calculations in 
	 * a background task. Also helps the scheduler be able to do some calculations asynchronously.
	 */
